...
impl Png {
    ...
    pub fn from_chunks(chunks: Vec<Chunk>) -> Self {
        Png { header: (Png::STANDARD_HEADER), chunks: (chunks) }
    }
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        match Png::try_from(v) {
            Ok(res) => Ok(res),
            Err(_) => Err(Box::new(PNGError)),
        }
    }
...
mod tests {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk> {
        ...
        let chunk_type_result = ChunkType::from_str(chunk_type);

        let chunk_type = match chunk_type_result {
            Ok(chunk_type) => chunk_type,
            Err(_) => return Err(Box::new(PNGError)),
        };
